...
- **`AssetChangeDetectionUtility.Enabled`**: a supported way to turn the `[DetectAssetChanged]` watcher on or off, with `ResetEnabledToDefault()` to drop the override and `EnabledScope(bool)` for a scoped change. Previously there was no opt-out at all short of removing the package from the editor assembly ([#327](https://github.com/Ambiguous-Interactive/unity-helpers/issues/327)).
- **`AssetChangeDetectionEnabledScope`**: an `IDisposable` that captures the watcher's enablement, applies an override, and restores exactly what it captured on dispose, so a bulk import cannot leak the override on an early return or an exception.
...
- **Bundled BCL assemblies competing for their own names**: the package ships `System.Text.Json`, `System.Text.Encodings.Web`, and `Microsoft.Bcl.AsyncInterfaces` at `9.0.0.0`, and Unity deduplicates precompiled assemblies by simple name, so exactly one copy wins each name project-wide. Two separate resolutions were wrong. Unity **6000.5** began shipping all three itself from `Editor/Data/BCLExtensions`, so those three are now constrained to `!UNITY_6000_5_OR_NEWER` and the editor's copies win there; every editor from 2021.3 through **6000.4** ships none of them and still needs the package's copies. Separately, a project that also installs a package shipping the same names -- `com.unity.ai.assistant` is the known case -- can hand the name to a copy marked `isExplicitlyReferenced`, which is then auto-referenced by nothing, leaving the name resolved by zero assemblies and failing the build with `CS0012` on `JavaScriptEncoder`. That collision is resolved in the consuming project; see [Bundled Assembly Conflicts](./docs/guides/bundled-assembly-conflicts.md). `System.IO.Pipelines`, which Unity never provides, still ships unconditionally ([#331](https://github.com/Ambiguous-Interactive/unity-helpers/issues/331)).
- **Headless editor runs crashing inside the asset-change watcher**: `[DetectAssetChanged]` discovery is an all-types / all-methods reflection scan, and running it inside Unity's import phase crashed a `-batchmode` editor natively (`STATUS_ACCESS_VIOLATION` in `GetMethodsByName_native`), after the run's real work had already succeeded. The existing guard for exactly this hazard only covered play mode. The watcher now declines to initialize in batch mode, where no author is present to act on a callback, and the gate sits on initialization rather than on the postprocess callback so the deferred-startup path is covered too ([#327](https://github.com/Ambiguous-Interactive/unity-helpers/issues/327)).
...
- **Missing Inspector for the cache-form dictionary**: `SerializableDictionary<TKey, TValue, TValueCache>` -- the documented answer for value types Unity cannot serialize directly -- had no registered property drawer, because it derives from `SerializableDictionaryBase` rather than from `SerializableDictionary<TKey, TValue>`. Following the documentation no longer costs the Inspector ([#314](https://github.com/Ambiguous-Interactive/unity-helpers/issues/314)).
- **Skipped ULF returns incorrectly proving Unity cleanup**: exact `Serial number unavailable for ULF return` messages now veto cleanup proof, including logs where another license group reports a successful ULF return. The portable and PowerShell classifiers require both the entitlement-return and client ULF-return success lines, and direct Unity runners no longer coerce or describe the skipped path as successful ([#310](https://github.com/Ambiguous-Interactive/unity-helpers/issues/310)).
