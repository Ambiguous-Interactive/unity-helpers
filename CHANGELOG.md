...
- Add `[WProtoSubtype(typeof(Base))]`, so a subtype joins a WallstopProto hierarchy with no field number and no edit to the base. **Assign WallstopProto Subtype Tags** commits the number and retires it on removal, so add / remove / re-add keeps the wire. Byte-identical to `[WProtoInclude]`. See [Polymorphism](./docs/features/serialization/serialization.md#polymorphism) ([#587](https://github.com/Ambiguous-Interactive/unity-helpers/issues/587), [#601](https://github.com/Ambiguous-Interactive/unity-helpers/issues/601)).
