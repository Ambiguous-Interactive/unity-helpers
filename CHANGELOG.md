...
- **`WProtoFacade` and the `WALLSTOP_PROTO` define**: `Serializer.ProtoSerialize` / `ProtoDeserialize` route a type through WallstopProto when the define is set **and** a formatter exists for the declared type that writes the value's runtime type, falling back to protobuf-net otherwise. Opt-in per type, so contracts can be ported and verified one at a time rather than all at once. An **empty** payload for a served type reads back as an all-defaults instance rather than an input failure, because a contract whose members all equal their defaults encodes to zero bytes. The define is off by default while the remaining contracts are ported ([#343](https://github.com/Ambiguous-Interactive/unity-helpers/issues/343)).
...
- **`[assembly: WProtoDeclaredRoot(typeof(IRandom), typeof(AbstractRandom))]`**: names the contract that serves a value held as an interface. A value held as `IRandom` now serializes through WallstopProto with the same bytes as before; declare your own pair for your own interfaces. A root you register with `Serializer.RegisterProtobufRoot` still wins. See [Declared roots](./docs/features/serialization/serialization.md#declared-roots-serving-an-interface) ([#403](https://github.com/Ambiguous-Interactive/unity-helpers/issues/403)).
