...
- **`IWProtoConditionalFormatter`**: a formatter registered for a type it cannot always serve — a root marshal over an element WallstopProto has no formatter for, such as an enum or a type protobuf-net reaches through a surrogate — reports that before anything is written, so the request falls back instead of failing ([#402](https://github.com/Ambiguous-Interactive/unity-helpers/issues/402)).
