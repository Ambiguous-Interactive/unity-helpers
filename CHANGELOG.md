...
- Add `[WProtoNotSerialized]`, and refuse an undeclared subclass of a `[WProtoContract]` at build time instead of throwing on the first save. Write it on a subclass that never reaches the serializer; the error names it beside the two ways to serialize one. See [A subclass that is not serialized](./docs/features/serialization/serialization.md#a-subclass-that-is-not-serialized) ([#613](https://github.com/Ambiguous-Interactive/unity-helpers/issues/613)).
