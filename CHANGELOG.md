...
- Add `TryNextUint`, `TryNextUlong`, `TryNextDouble` and `TryNextGaussian` to `AbstractRandom`, which report a stalled entropy source instead of returning the degraded answer the existing methods give. A generator still never throws for its own internal state ([#638](https://github.com/Ambiguous-Interactive/unity-helpers/issues/638)).
...
- Add `WallstopProto` (preview), a reflection-free protobuf reader and writer that AOT-compiles under IL2CPP and is byte-compatible with protobuf-net. Hand-written formatters are accepted only when they consume their complete payload without latching a malformed read. See [WallstopProto](./docs/features/serialization/serialization.md#wallstopproto-the-reflection-free-wire-layer-preview) ([#343](https://github.com/Ambiguous-Interactive/unity-helpers/issues/343), [#647](https://github.com/Ambiguous-Interactive/unity-helpers/issues/647)).
