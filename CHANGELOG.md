...
- Add AOT support for `ValueTuple` in protobuf and JSON: both threw `ExecutionEngineException` on an IL2CPP player and now go through generated formatters and hand-written converters. Define `WALLSTOP_DISABLE_VALUE_TUPLE_SERIALIZATION` to opt out. See [Tuples serialize on IL2CPP](./docs/features/serialization/serialization-types.md#tuples-serialize-on-il2cpp) ([#289](https://github.com/Ambiguous-Interactive/unity-helpers/issues/289)).
