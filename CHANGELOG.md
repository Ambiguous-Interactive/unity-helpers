...
- **A pooled buffer could be handed to two callers at once**: copying a `PooledResource<T>` — passing it to a method, assigning it, capturing it — and disposing both copies returned the same instance to the pool twice, so two live rentals shared one list and every write through one appeared in the other. A resource is now returned at most once per rent, however many copies exist. Everything built on `Buffers<T>`, `SetBuffers<T>` and `DictionaryBuffer<TKey, TValue>` is covered ([#358](https://github.com/Ambiguous-Interactive/unity-helpers/issues/358)).
- **`AssetDatabaseBatchScope` disposed twice ended asset editing early**: each extra disposal decremented the batch depth again, so an outer scope could find batching already stopped, or auto-refresh left off. Disposing a scope more than once is now a no-op ([#358](https://github.com/Ambiguous-Interactive/unity-helpers/issues/358)).
