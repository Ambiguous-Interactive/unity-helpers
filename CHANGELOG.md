...
- Bound a rectangular array's dimensions individually: an empty one whose axis exceeds `SerializationCapacityLimits.MaximumRestoredCapacity` no longer deserializes. Arrays that carry elements are unaffected, and only a hand-written caller of `WProtoRectangular.TryAcceptShape` is affected ([#437](https://github.com/Ambiguous-Interactive/unity-helpers/issues/437)).
