...
- Add `ApproximatelyEquals(other, tolerance)` to `Circle`, `Sphere`, `Line2D`, `Line3D` and `PoolFrequencyStatistics`, so a computed shape can still be compared loosely now that `Equals` is exact. A negative, infinite or NaN tolerance returns false ([#639](https://github.com/Ambiguous-Interactive/unity-helpers/issues/639)).
...
- Restrict `Equals(object)` on `Attribute`, `WGuid`, `FastVector2Int`, `FastVector3Int`, `SerializableType` and `SerializableNullable<T>` to its own type. A boxed `float`, `Guid`, `Type` or `Vector2Int` never answered true in return. The strongly typed overloads are unchanged ([#639](https://github.com/Ambiguous-Interactive/unity-helpers/issues/639)).
...
- Fix `AttributeEffect.Equals` ignoring `periodicEffects`, `behaviors` and all four stacking fields, and its hash ignoring the name it compares. Both cover every authored field now ([#639](https://github.com/Ambiguous-Interactive/unity-helpers/issues/639)).
...
- Fix `default(RandomState)` hashing zero while comparing equal to `new RandomState(0)`. The hash comes from the members `Equals` compares, never from the value a payload carried ([#639](https://github.com/Ambiguous-Interactive/unity-helpers/issues/639)).
