...
- Add `WUH003` and `WUH004`, which warn when `?.`, `??` or `??=` is applied to a `UnityEngine.Object` -- those operators test CLR null, so a destroyed object walks straight through the guard -- and when a null assertion passes over one. See [Analyzers](./docs/performance/analyzers.md) ([#621](https://github.com/Ambiguous-Interactive/unity-helpers/issues/621)).
