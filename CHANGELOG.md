...
- **Bundled BCL assemblies competing with Unity 6000's own**: the package shipped `System.Text.Json`, `System.Text.Encodings.Web`, and `Microsoft.Bcl.AsyncInterfaces` at `9.0.0.0` while Unity 6000 ships all three itself at `8.0.0.0`. Which copy the compiler bound was left to Unity's dedup, and a mixed resolution failed a consuming project's build with `CS0012` on `JavaScriptEncoder` or `CS0103` on `Unsafe`. Those three are now constrained to editors that do not ship them (`!UNITY_6000_5_OR_NEWER`); `System.IO.Pipelines`, which Unity does not provide, still ships unconditionally ([#331](https://github.com/Ambiguous-Interactive/unity-helpers/issues/331)). Unity supplies them from `Editor/Data/BCLExtensions`, which first appears in **6000.5** -- every editor from 2021.3 through 6000.4 still needs the package's copies.
