...
- Add `WPROTO038`, a warning when a method group is passed as the factory to `ConcurrentDictionary.GetOrAdd`, `AddOrUpdate` or `ConditionalWeakTable.GetValue`. C# rebuilds that delegate on every lookup, cache hits included. It is a warning at most and can be turned off in a ruleset or `.editorconfig`. See [The generator](./docs/features/serialization/serialization.md#the-generator) ([#538](https://github.com/Ambiguous-Interactive/unity-helpers/issues/538)).
