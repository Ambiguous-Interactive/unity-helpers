...
- **Set items silently lost**: `SerializableHashSet<List<TValue>>` saved nothing. The Inspector now reports this instead of drawing a column that persists nothing; use `SerializableHashSet<SerializableList<TValue>>` if you want a set keyed on list identity ([#314](https://github.com/Ambiguous-Interactive/unity-helpers/issues/314)).
