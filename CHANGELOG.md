...
- Add `Span<T>` shuffling -- `Shuffle`, `TryCopyShuffled` and `TryGetRandomElement` -- so a shared table can be shuffled into a `stackalloc` buffer with no allocation. It draws from the generator exactly as the `IList` overload does, so seeded output is unchanged. See [Span Operations](./docs/features/utilities/math-and-extensions.md#span-operations) ([#632](https://github.com/Ambiguous-Interactive/unity-helpers/issues/632)).
...
- Add the **Asset Validation** window -- **Tools > Wallstop Studios > Unity Helpers** -- a dockable results view with search, a severity floor, click-to-ping, and one-click suppression. Tick **Re-check on import** and an import re-validates only the assets it touched ([#288](https://github.com/Ambiguous-Interactive/unity-helpers/issues/288)).
