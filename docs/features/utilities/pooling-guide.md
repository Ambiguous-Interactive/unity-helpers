...
actually used. Those samples go into a fixed ring of 64 time buckets covering `RollingWindowSeconds`,
allocated once when the pool is constructed: recording is O(1), never allocates, and costs the same
