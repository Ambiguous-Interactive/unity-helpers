...
namespace WallstopStudios.UnityHelpers.Core.Serialization.WallstopProto
{
    ...
    public ref struct WProtoReader
    {
        ...
        public bool TryReadTag(out int fieldNumber, out int wireType)
        {
            ...
            // Reject keys above 32 bits without rejecting redundant padding accepted by protobuf readers.
        ...
        public bool TryReadVarint64(out ulong value)
        {
            ...
            for (int shift = 0; shift < WProtoSizes.MaxVarintBytes; shift++)
            {
                ...
                // Only the highest bit of a 64-bit value fits in the tenth varint byte.
        ...
        /// </summary>
        /// <param name="value">Receives a view over the remaining bytes, or an empty span on failure.</param>
        /// <returns><c>true</c> when bytes remain on a valid reader; <c>false</c> for an empty region
        /// or after the reader became malformed.</returns>
        /// <remarks>
        ...
        private bool TrySkipGroup(int groupFieldNumber, int depth)
        {
            ...
            while (true)
            {
                ...
                {
                    // The terminator must name this group; mismatches would accept crossed nesting.
