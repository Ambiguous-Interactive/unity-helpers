...
8. **Aim for zero comments.** Reach for a better name before a better sentence, and spell names out rather than abbreviating. A comment that survives that explains **why**, never **what**; a non-doc comment spanning more than one line uses the `/* ... */` block form (see [create-csharp-file](./skills/create-csharp-file.md))
