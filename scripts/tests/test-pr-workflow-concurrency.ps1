#!/usr/bin/env pwsh
# Contract test: pull_request workflows must cancel superseded PR iterations,
# including licensed workflows whose cancellation-safe cleanup releases the lock.
[CmdletBinding()]
...
Write-Host "[test-pr-workflow-concurrency] OK: pull_request concurrency cancels superseded runs, including licensed workflows with cancellation-safe cleanup." -ForegroundColor Green
exit 0
